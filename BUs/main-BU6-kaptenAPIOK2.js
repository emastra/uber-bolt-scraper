...
Apify.main(async () => {
  ...
  const crawler = new Apify.PuppeteerCrawler({
    ...
    gotoFunction: async ({ request, page }) => {
      ...
      const cookie = request.userData.cookie;
      if (cookie) {
    ...
    handlePageFunction: async ({ page, request, response }) => {
      ...
      if (label === 'LOGIN') {
        ...
        if (site === 'kapten') {
          ...
          const fromLat = fromCoords.split(',')[0];
          const fromLng = fromCoords.split(',')[1];
          const toLat = toCoords.split(',')[0];
          const toLng = toCoords.split(',')[1];
